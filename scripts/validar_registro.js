document.addEventListener("DOMContentLoaded", function () {
  ...
  formulario.addEventListener("submit", function (event) {
    ...
    const apellidos_usuario = formulario.querySelector(
      "[name='apellidos_usuario']"
    );
    const mail_usuario = formulario.querySelector("[name='mail_usuario']");
    const telefono_usuario = formulario.querySelector(
    ...
    if (
      !regexNombreApellido.test(nombre_usuario.value) ||
      nombre_usuario.value.trim() === ""
    ) {
      mostrarError(
        nombre_usuario,
        "El campo nombre solo puede contener letras, espacios y es obligatorio."
      );
      error = true;
    }
    ...
    if (
      !regexNombreApellido.test(apellidos_usuario.value) ||
      apellidos_usuario.value.trim() === ""
    ) {
      mostrarError(
        apellidos_usuario,
        "El campo apellidos solo puede contener letras, espacios y es obligatorio."
      );
      error = true;
    }
    ...
    if (
      !regexTelefono.test(telefono_usuario.value) ||
      telefono_usuario.value.trim() === ""
    ) {
      mostrarError(
        telefono_usuario,
        "El campo teléfono solo puede contener números y es obligatorio."
      );
      error = true;
    }
